...
def get_comment_or_404(db: Session, user: User, comment_id: int) -> TaskComment:
    comment = db.get(TaskComment, comment_id)
    if not comment or not access.task_visible(db, user, comment.task):
...
def list_comments(task_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    return [to_schema(c) for c in access.require_task(db, current_user, task_id).comments]
...
def search_comments(q: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    ids = {task_id for (task_id,) in db.query(TaskComment.task_id).filter(TaskComment.body.ilike(pattern, escape="\\"))}
    tasks = db.query(Task).filter(Task.id.in_(ids)).all() if ids else []
    return sorted(t.id for t in access.filter_tasks(db, current_user, tasks))
