...
# Retention comes from the app's settings table (Settings page); fall back to
# 3 if it isn't set or the table doesn't exist yet (pre-0003 databases).
if [[ -z "${PM_BACKUP_KEEP:-}" ]]; then
  PM_BACKUP_KEEP="$(PGPASSWORD="$DB_PASSWORD" psql -h "${DB_HOST:-localhost}" -p "${DB_PORT:-5432}" \
    -U "${DB_USER:-projectmanager}" -d "${DB_NAME:-projectmanager}" -qtAX \
    -c "SELECT value FROM app_settings WHERE key = 'backup_keep'" 2>/dev/null || true)"
fi
